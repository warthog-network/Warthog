...
constexpr inline uint32_t byte_swap(uint32_t x)
{
    return (((x & 0xff000000U) >> 24) | ((x & 0x00ff0000U) >> 8) | ((x & 0x0000ff00U) << 8) | ((x & 0x000000ffU) << 24));
}

constexpr inline uint16_t byte_swap(uint16_t x)
{
    return (x >> 8) | (x << 8);
}

constexpr inline uint8_t byte_swap(uint8_t x)
{
    return x;
}
