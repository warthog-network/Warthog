...
MinerPort::MinerPort(std::span<uint8_t, 80> argnew, HashView target,
                   uint32_t seedOffset)
    : target(target), seedOffset(seedOffset) {
  set_header(argnew);
}
void MinerPort::set_header(std::span<uint8_t, 80> header) {
  std::copy(header.begin(), header.end(), arg.begin());
  vh.reset();
  vh.write(arg.data(), arg.size(), haraka512_port);
...
auto MinerPort::mine(uint32_t count) -> std::optional<Success> {
  ...
  for (i = seedOffset; i != end; i++) {
    ...
    *(u128 *)(curBuf + 32 + 16) = *(u128 *)(curBuf);
    ...
    const uint64_t intermediate = precompReduction64_port(acc);

    *(uint64_t *)(curBuf + 32 + 16) = intermediate;
    *(uint64_t *)(curBuf + 32 + 16 + 8) = intermediate;

    haraka512_port_keyed_local(curHash.data(), curBuf,
