...
Element border(Element child) {
  return std::make_shared<Border>(Elements{std::move(child)}, ROUNDED);
}
...
Decorator borderWith(const Pixel &pixel) {
  return [pixel](Element child) {
    return std::make_shared<BorderPixel>(Elements{std::move(child)}, pixel);
  };
}
...
Decorator borderStyled(BorderStyle style) {
  return [style](Element child) {
    return std::make_shared<Border>(Elements{std::move(child)}, style);
  };
}
...
Element borderDashed(Element child) {
  return std::make_shared<Border>(Elements{std::move(child)}, DASHED);
}
...
Element borderLight(Element child) {
  return std::make_shared<Border>(Elements{std::move(child)}, LIGHT);
}
...
Element borderHeavy(Element child) {
  return std::make_shared<Border>(Elements{std::move(child)}, HEAVY);
}
...
Element borderDouble(Element child) {
  return std::make_shared<Border>(Elements{std::move(child)}, DOUBLE);
}
...
Element borderEmpty(Element child) {
  return std::make_shared<Border>(Elements{std::move(child)}, EMPTY);
}
...
Element window(Element title, Element content, BorderStyle border) {
  return std::make_shared<Border>(
      Elements{std::move(content), std::move(title)}, border);
}
} // namespace ftxui
