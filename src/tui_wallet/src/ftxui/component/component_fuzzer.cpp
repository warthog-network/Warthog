...
Component GeneratorComponent(const char *&data, size_t &size, int depth) {
  ...
  case 0:
    return Button(
        GeneratorString(data, size), [] {}, GeneratorButtonOption(data, size));
  case 1:
  ...
  case 6:
    return Slider(GeneratorString(data, size), &g_int, GeneratorInt(data, size),
  ...
  case 9:
    return ResizableSplitTop(GeneratorComponent(data, size, depth - 1),
                             GeneratorComponent(data, size, depth - 1), &g_int);
  case 10:
