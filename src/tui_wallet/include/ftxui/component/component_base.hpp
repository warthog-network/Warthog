...
#include "captured_mouse.hpp" // for CaptureMouse
...
class ComponentBase;
class Component : public std::shared_ptr<ComponentBase> {
public:
  using std::shared_ptr<ComponentBase>::shared_ptr;
  Component(std::shared_ptr<ComponentBase> ptr)
      : std::shared_ptr<ComponentBase>(std::move(ptr)) {};
};
...
inline Element::Element(std::shared_ptr<C> p) : std::shared_ptr<Node>(p->Render()) {}

// add constructor to convert components into element
inline Element::Element(Component &c) : Element(c->Render()) {};
} // namespace ftxui
