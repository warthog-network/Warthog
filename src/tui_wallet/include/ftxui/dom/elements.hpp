...
class Element : public std::shared_ptr<Node> {
public:
  Element(Component &c);

  template <typename C>
    requires(std::is_base_of_v<ComponentBase, C>)
  Element(std::shared_ptr<C> p);

  Element(std::shared_ptr<Node> p) : std::shared_ptr<Node>(std::move(p)) {}
...
// //
// template <class T> void Merge(Elements & /*container*/, T /*element*/) {}
//
// template <> inline void Merge(Elements &container, Element element) {
//   container.push_back(std::move(element));
// }
//
// template <> inline void Merge(Elements &container, Elements elements) {
...
// template <class... Args> Elements unpack(Args... args) {
//   return std::vector<Element>({Element(args)...});
// }
template <class T> void Merge(Elements & /*container*/, T /*element*/) {}
...
#define TAKE_ANY_ARGS(container)                                               \
  template <class... Args>                                                     \
    requires((std::is_same_v<Args, std::vector<Element>> || ...) == false)     \
  Element container(Args... children) {                                        \
    return container(Elements(std::vector<Element>{Element(children)...}));    \
  }
