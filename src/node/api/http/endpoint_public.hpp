...
struct Config;
class HTTPEndpointPublic {
    struct Token {
    };
public:
    static std::unique_ptr<HTTPEndpointPublic> make_endpoint_public(const Config&);
    ~HTTPEndpointPublic()
    {
        if (t.joinable()) {
            lc.loop->defer(std::bind(&HTTPEndpointPublic::shutdown, this));
            t.join();
        }
    }
    ...
    HTTPEndpointPublic(Token,const Config&);
private:
    void async_reply(uWS::HttpResponse<false>* res, std::string reply)
    {
        lc.loop->defer(std::bind(&HTTPEndpointPublic::send_reply, this, res, std::move(reply)));
    }
    void work();
    void shutdown();
