...
uint64_t c0(uint64_t x, NonzeroHeight h)
{
    uint64_t y = x;
    if (h.value() >= NEWBLOCKSTRUCUTREHEIGHT) { // try out new c basis
        y += 0xfffffffffffea07e;
        y *= x;
        y += 0x0000000005a02adaf;
        y *= x;
        y += 0xffffff50c790bca2;
    } else {
...
uint64_t c1(uint64_t x, NonzeroHeight h)
{
    uint64_t y = x;
    if (h.value() >= NEWBLOCKSTRUCUTREHEIGHT) { // try out new c basis
        y += 0xfffffffffffdcd35;
        y *= x;
        y += 0x00000001363eb0c7;
        y *= x;
        y += 0xfffffc9bf08d7233;
    } else {
...
void log_compressed(const TransferView& t, NonzeroHeight h)
{
    ...
    auto c_0 { c0(t.fromAccountId().value(), h) };
    auto c_1 { c1(t.fromAccountId().value(), h) };
...
{
    // log compression of Transfer items, this may be implemented in future to further decrease byte size of items
    auto c_0 { c0(t.from_id().value(), h) };
    auto c_1 { c1(t.from_id().value(), h) };
    assert_no_abort(c_0 != 0);
    assert_no_abort(c_1 != 0);
