...
public:
    ...
    ~TemplatedApp() {
        ...
        if (topicTree) {
            ...
            l->removePostHandler(topicTree);
            l->removePreHandler(topicTree);
        }
    }
    ...
    TemplatedApp(Loop* l,SocketContextOptions options = {}):l(l) {
        httpContext = HttpContext<SSL>::create(l, options);
    }
    ...
    TemplatedApp &&ws(std::string pattern, WebSocketBehavior<UserData> &&behavior) {
        ...
        if (!topicTree) {
            ...
            l->addPostHandler(topicTree, [topicTree = topicTree](Loop */*loop*/) {
                /* Commit pub/sub batches every loop iteration */
                topicTree->drain();
            });

            l->addPreHandler(topicTree, [topicTree = topicTree](Loop */*loop*/) {
                /* Commit pub/sub batches every loop iteration */
                topicTree->drain();
            });
        }
        ...
        auto *webSocketContext = WebSocketContext<SSL, true, UserData>::create(l, (us_socket_context_t *) httpContext, topicTree);
